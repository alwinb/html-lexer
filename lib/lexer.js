...
// emit interface. Keeps track of the
...
function Lexer (StateMachine, specials) { return function () {
  ...
  //  this is separate from tokenize as files may be 
  //  split in chuncks before being tokenized. 
  ...
  function tokenize (input, cb) {
    // This is tricky.. if a token spans chunck boundaries..
    // then they need to be glued together again first..
    init () 
  	var last = 0
    var pos = 0
  	var finished = false
  	var sm = new StateMachine (advance, emit)
