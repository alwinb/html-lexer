...
charRef:
  [ { if: '&#[0-9]+;?',      emit:'decimalCharRef',     goto: context }
	, { if: HEX_CHARREF,       emit:'hexadecimalCharRef', goto: context }
	, { if: '&[A-Za-z0-9]+;?', emit:'maybeNamedCharRef',  goto: context }
	, { if: '&',               emit:'unescapedAmpersand', goto: context }
	],
...
function Lexer (input, customState) {
	var symbol = 'data' // The state name
  ...
  this[Symbol.iterator] = _ => this
    ...
    symbol = (typeof edge.goto === 'function')
      ? edge.goto.call (customState, symbol, chunk) // want access to.. what? and allow to set.. what?
      : edge.goto
...
function lex (input) {
  return new Lexer (input, { content:'data' })
}
