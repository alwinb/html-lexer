...
function StateMachine (stream, emit, error) {
var state = { state:'data', quotation:null, tagName:'', tagType:null, stack:[], prefixCount:0 }
...
beforeAttributeValue: function (peek) { // 'after equals'
	if (peek === EOF) {
		...
		if (peek === '<' || peek === '=' || peek === '`')
			error('attribute values must not start with a '+peek+' character')
		emit('equals')
		emit('beginAttributeValue')
		this.state = 'attributeValue'
			this.quotation = ''
...
namedCharRef: function (peek) {
	if (peek !== EOF && ALPHANUM.test(peek))
		...
		consume()
		emit('namedCharRef', { inAttribute:(this.stack[0] === 'attributeValue') })
		popState()
	}
	else {
		// Branch does not consume
		emit('namedCharRef', { inAttribute:(this.stack[0] === 'attributeValue'), nextChar:peek })
		popState()
	}
}
...
function run () {
	...
	states[state.state].call(state, stream.peek()) }
