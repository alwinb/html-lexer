...
var state = { state:'data', context:null, tagName:'', tagType:null }
...
attributeValueDoubleQuoted: function (peek) {
	if (peek === EOF) {
		emitError(eof_msg+'in attribute value')
		emit('attributeValueDoubleQuoted')
	}
	else if (peek === '"') {
		consume()
		emit('attributeValueDoubleQuoted')
	...
	// NB this branch is not needed for the template language. 
	// To implement this, several token types need to be added,
	// e.g. attributeData, attributeStart, etc
	// else if (peek === '&') {
	//	emit('attributeValueDoubleQuoted')
	//	consume()
	//	this.state = 'charRefIn_'
	//		this.charRefIn_ = 'attributeValueDoubleQuoted'
	// }
	else {
		consume()
	}
...
attributeValueSingleQuoted: function (peek) {
	if (peek === EOF) {
		emitError(eof_msg+'in attribute value')
		emit('attributeValueSingleQuoted')
	}
	else if (peek === "'") {
		consume()
		emit('attributeValueSingleQuoted')
