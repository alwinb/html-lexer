...
function StateMachine (consume, emit, error) {
...
namedCharRef: function (peek) {
	if (peek !== EOF && ALPHANUM.test(peek))
		...
		consume()
		emit('namedCharRef', { inAttribute:(this.stack[this.stack.length-1] === 'attributeValue') })
		this.popState()
	}
	else {
		// Branch does not consume
		emit('namedCharRef', { inAttribute:(this.stack[this.stack.length-1] === 'attributeValue'), nextChar:peek })
		this.popState()
	}
}
